      ...
      <AnimatePresence mode="popLayout">
        {tasks.map((task, index) => (
          <motion.article
            ...
            className="rounded-lg border border-white/10 bg-white/[0.08] px-3 py-2.5 shadow-sm shadow-[#0f2326]/10 transition hover:border-white/20 hover:bg-white/[0.12]"
            exit={{ opacity: 0, x: 12 }}
          ...
          >
            <div className="flex items-center gap-2">
              {task.tag ? (
                <span
                  className="h-8 w-1.5 shrink-0 rounded-full shadow-sm shadow-black/20"
                  style={{ backgroundColor: task.tag }}
                />
              ) : null}
              <div className="min-w-0 flex-1">
                <h2 className="truncate text-sm font-semibold text-white">{task.name}</h2>
                <p className="mt-0.5 line-clamp-2 text-xs leading-5 text-white/62">
                  {task.info}
                </p>
              </div>
            </div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-1">
              ...
              <button
                className="grid h-8 place-items-center rounded-md border border-emerald-100/20 bg-emerald-100/10 text-emerald-50 transition hover:bg-emerald-100/20"
