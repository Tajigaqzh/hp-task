...
export function DesktopWidgetHeader({
  ...
  return (
    <header
      className={`flex items-center justify-between border-b border-white/10 bg-white/[0.04] px-4 py-3 ${
        pinned ? "cursor-default" : "cursor-move"
      }`}
      onMouseDown={onDragStart}
    >
      <div className="flex min-w-0 items-center gap-3">
        <GripHorizontal className="h-4 w-4 shrink-0 text-[#8fc8bd]" />
        <motion.div
          animate={{ opacity: 1, x: 0 }}
          initial={{ opacity: 0, x: -8 }}
          transition={{ delay: 0.08, duration: 0.2 }}
        >
          <p className="text-xs font-semibold uppercase text-white/50">HP Task</p>
